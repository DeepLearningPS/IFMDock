    ...
    def Load(self,modelname,loadtype='Perepoch'):
        ...
        with tempfile.TemporaryDirectory() as dirpath:
            ...
            if loadtype=='Minloss':
                ...
                modelcpkt=torch.load(f"{dirpath}/model/online_model_minloss.cpk") #online_model_minloss
                self.online_model.load_state_dict(parallel_cpkt_to_single(modelcpkt)) #因为训练是在多卡GPU上的，而生成实在单卡上的，所以加载多卡的模型需要单卡需要特殊处理一下
    ...
    def sample_batch(self,Datas):
        ...
        with torch.no_grad():
            ...
            if GP.final_timesteps == 1:
                sigmas= reversed(sigmas)  #此时sigmas的长度为1，即只有一个数，sigmas越大，噪音越大
                sigmas[-1] = 80 #如果不设置为80即sigma的最大值，则当长度为1时，经过karras_schedule()得到的sigma 非常小，基本上等于没噪音，显然是不合理的，因为此时去噪模型也没什么用了
            else:
        ...
        if GP.multi_step == 1:
            ...
            for stp in list(total_samples.keys()):
                ...
                for i in range(len(Final_MGs)):    #处理每一个构象
                    ...
                    try:
                        mol=mg.Trans_to_Rdkitmol() #转化成rdkit mol对象，这里会改变原子顺序 #我们通过其它方法获取新的mol
                        #mol=mg.update_mol()
                        mols.append(mol)
