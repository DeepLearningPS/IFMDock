...
class ConsistencyTraining:
    ...
    def __call__(
        ...
        **kwargs : Any
        ...
        cross_distance_loss = F.mse_loss(c_lp_distance[mask_cutoff], n_lp_distance[mask_cutoff]) #截断之后，矩阵变成了向量，现在有一个问题，截断后变成了向量， 怎么除批量？
...
class ConsistencySamplingAndEditing:
    ...
    def __call__(
        ...
        **kwargs : Any
        ...
        time_list = np.array(list(reversed(list(range(len(sigmas)))))) + 1 #记录时间步长，最好从1开始。从0开始，会导致编码失效的
