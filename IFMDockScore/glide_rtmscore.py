...
if __name__ == '__main__':
    ...
    for dt_name in data_name_list[inargs.st_i: inargs.en_i]:
        #/mnt_191/fanzhiguang/47/VSDS/VSDS_Glide/glide/O00329-6PYR/model/ecdock_step5/gen_docking/active0/origin_ligand_active0_config_lib.sdf
        dt_dir = os.path.join(inargs.data_dir, f'{dt_name}', 'model/ecdock_step5/gen_docking')
        ...
        for dt in tqdm(list(sorted(list(os.listdir(dt_dir))))[:]):
            ...
            if os.path.exists(path) and os.path.isdir(path) and os.listdir(path) and 'model' not in dt: #目录存在且不空
                ...
                if inargs.refine_flag == 'refine':
                    ...
                    l_file = os.path.join(path, f'origin_ligand_{dt}_config_lib.sdf') 
