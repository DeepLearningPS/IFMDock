...
def joint_bar(active_num_atoms_list, predict_num_atoms_list, save_file, x_title=None, bins=10):
    ...
    x = np.linspace(xmin, xmax, 100)  # 在x轴上创建100个点
    p = norm.pdf(x, mu, std)  # 计算这些点对应的正态分布概率密度
    plt.plot(x, p, linewidth=2, label=f'NonGap-76: $\mu$={mu:.2f}, $\sigma$={std:.2f}')
    ...
    #x = np.linspace(xmin, xmax, 100)  # 在x轴上创建100个点
    p = norm.pdf(x, mu, std)  # 计算这些点对应的正态分布概率密度
    plt.plot(x, p, linewidth=2, label=f'Gap-71: $\mu$={mu:.2f}, $\sigma$={std:.2f}')
...
if __name__ == '__main__':
    ...
    with open('/data/fan_zg/MDocking/new_VSDS/vsds_gap_71.txt') as f:
        for line in f:
            gap_set.add(line.strip())
    ...
    all_set = set(EF_dict.keys())

    non_gap_set = all_set - gap_set
    
    print('len(gap_set):', len(gap_set))
    print('len(non_gap_set):', len(non_gap_set))
    print('len(all_set):', len(all_set))


            
    pos_diff_names = list(non_gap_set)
    neg_diff_names = list(gap_set)
    
    

    flag_name = 'non_gap-gap-carsidock'
    ...
    EF_case_analy(pos_active_dict, neg_active_dict, EF_base, data_base, flag_name)
