...
def EF_case_analy(active_dict, predict_TopN_dict, pdbbind_atom_count_list, EF_base, data_base, flag_name = None):
    # ─── 数据加载 ───────────────────────────────────────
    ...
    # 调用 bar 函数绘制图像
    joint_bar(active_num_atoms_list, predict_num_atoms_list, pdbbind_atom_count_list, save_file, x_title='The number of atoms', bins=10)
...
if __name__ == '__main__':
    ...
    flag_name = 'pos-neg-carsidock-pdbbind'
    
    
    pos_active_dict       = {}
    neg_active_dict       = {}
    
    for data_name in tqdm(pos_diff_names[0:150]):
        ...
        print('active N:', N)
        pos_active_dict[data_name]       = active_list
        

    for data_name in tqdm(neg_diff_names[0:150]):
        '''返回富集率dict，然后排序，前N个， N等于实际的活性分子的数量'''
        data_path = os.path.join(base_dir, model, f'{data_name}')
        N = 0
        active_list = []
        ...
        print('active N:', N)
        neg_active_dict[data_name]       = active_list     
        
    print('pos_active_dict:', pos_active_dict)
    print('neg_active_dict:', neg_active_dict)
    ...
    EF_case_analy(pos_active_dict, neg_active_dict, pdbbind_atom_count_list, EF_base, data_base, flag_name)
