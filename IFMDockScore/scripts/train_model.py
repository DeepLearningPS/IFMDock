...
'''如何处理成dgl图对象：这个文件中有，需要提供rdkit mol对象。RTMScore/feats/mol2graph_rdmda_res.py，这样，
我们仅在第一次时使用rdkit处理成dgl图对象，并保存成pickle文件，之后直接加载pickle文件即可。制作一个控制按钮'''

data = PDBbindDataset(ids="%s/%s_ids.npy"%(args["data_dir"], args["train_prefix"]), #数据id
                      ligs="%s/%s_l.bin"%(args["data_dir"], args["train_prefix"]), #数据pickle文件，存储了dgl图对象，因此需要提前处理成dgl对象并保存成pickle文件
                      prots="%s/%s_p.bin"%(args["data_dir"], args["train_prefix"])
                      )

train_inds, val_inds = data.train_and_test_split(valnum=args["valnum"], seed=args['seeds']) #根据id，分割训练集和验证集
