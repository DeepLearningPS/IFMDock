...
if __name__ == '__main__':
    ...
    for dt_name in data_name_list[inargs.st_i: inargs.en_i]:
        #if dt_name not in all_lost_data_name[2:3]:
            #continue
        #/mnt_191/fanzhiguang/47/mnt/CarsiDock/outputs/vsds/O00329-6PYR/active0/gen_active0_ligand.sdf
        dt_dir = os.path.join(inargs.data_dir, f'{dt_name}')
        ...
        for dt in tqdm(list(sorted(list(os.listdir(dt_dir))))[:]):
            ...
            if os.path.exists(path) and os.path.isdir(path) and os.listdir(path) and 'model' not in dt: #目录存在且不空
                #需要改 #/data/fan_zg/MDocking/VSDS_DTEBV-D/data/O00329-6PYR/O00329-6PYR/active0/active0_protein.pdb
                ...
                if inargs.refine_flag == 'refine':
                    l_file = os.path.join(path, f'glide_refine_gen_{dt}_addh.sdf')  #glide_refine_gen_5SB2_addh.sdf
                    if not os.path.exists(l_file):
                        print('refine is 不存在')
                        l_file = os.path.join(path, f'{dt}_addh_gen_ligand.sdf')
                else:
                    l_file = os.path.join(path, f'gen_{dt}_addh.sdf') 
                ...
                if inargs.refine_flag == 'refine':
                    rtm_score_l_file   = os.path.join(path, f'refine_rtm_sort_gen_ligand_{dt}.sdf')
                else:
                    rtm_score_l_file   = os.path.join(path, f'rtm_sort_gen_ligand_{dt}.sdf')
