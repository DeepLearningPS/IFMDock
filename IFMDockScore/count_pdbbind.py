...
# 修改后的 bar 函数
def joint_bar(active_num_atoms_list, predict_num_atoms_list, pdbbind_atom_count_list, save_file, x_title=None, bins=10):
    # 绘制两个分布的直方图
    plt.figure(figsize=(8, 6))  # 设置图表大小
    ...
    new_list = active_num_atoms_list + predict_num_atoms_list + pdbbind_atom_count_list
    ...
    x = np.linspace(xmin, xmax, 100)  # 在x轴上创建100个点
    p = norm.pdf(x, mu, std)  # 计算这些点对应的正态分布概率密度
    plt.plot(x, p, linewidth=2, label=f'Good-20: $\mu$={mu:.2f}, $\sigma$={std:.2f}')
    ...
    #x = np.linspace(xmin, xmax, 100)  # 在x轴上创建100个点
    p = norm.pdf(x, mu, std)  # 计算这些点对应的正态分布概率密度
    plt.plot(x, p, linewidth=2, label=f'Poor-20: $\mu$={mu:.2f}, $\sigma$={std:.2f}')
    


    # 拟合 pdbbind_atom_count_list 为正态分布
    #mu_predict, std_predict = norm.fit(pdbbind_atom_count_list)
    #n, bins, patches = plt.hist(pdbbind_atom_count_list, bins=bins, edgecolor='black', alpha=0.6, color="#6f0b2b", density=True)
    mu, std = norm.fit(pdbbind_atom_count_list)  # 获取正态分布的均值和标准差
    # 绘制拟合曲线（正态分布）
    #xmin, xmax = plt.xlim()  # 获取x轴的范围
    #x = np.linspace(xmin, xmax, 100)  # 在x轴上创建100个点
    p = norm.pdf(x, mu, std)  # 计算这些点对应的正态分布概率密度
    plt.plot(x, p, linewidth=2, label=f'Training set: $\mu$={mu:.2f}, $\sigma$={std:.2f}')
...
def EF_case_analy(mol_list, save_path, flag_name = None):
    # ─── 数据加载 ───────────────────────────────────────
    ...
    for mol in mol_list:
        try:
            num_atoms, morgan_fp = analyze_top_EF(mol)
        except Exception as e:
    ...
    # 调用 bar 函数绘制图像
    bar(active_logP_list, save_file, x_title='The RDKit LogP Hydrophobicity', bins=10)   
...
if __name__ == '__main__':
    ...
    with open(f'/data/fan_zg/MDocking/EcDock_Evaluate/EF/pdbbind/pdbbind_atom_count_list.pickle', 'rb') as f:
        pdbbind_atom_count_list = dill.load(f)


    
    EF_base = '/data/fan_zg/MDocking/EcDock_Evaluate/EF/EF-case'
    EF_dict = {}
    with open(f'{EF_base}/carsidock.pickle', 'rb') as f:
        EF_dict = dill.load(f)
    ...
    # 统计pdbbind数据集中配体的原子数分布
    pdbbind_dir = "/data/fan_zg/MDocking/CrossDocked2020/data/newer_pdbbind2020/newer_pdbbind2020"
    
    #/data/fan_zg/MDocking/CrossDocked2020/data/newer_pdbbind2020/newer_pdbbind2020/11gs/11gs_ligand.sdf
    
    atom_count_list = []
    atom_list = []
    for k1 in os.listdir(pdbbind_dir):
        pdbbind_sdf = os.path.join(pdbbind_dir, k1, f'{k1}_ligand.sdf')
        
        if os.path.exists(pdbbind_sdf):
            mol = Chem.SDMolSupplier(pdbbind_sdf)[0]
            if mol is not None:
                atom_count = mol.GetNumAtoms()
                atom_count_list.append(atom_count)
                atom_list.append(mol)
            else:
                continue
    ...
    '''分析pdbbind'''
    save_path = '/data/fan_zg/MDocking/EcDock_Evaluate/EF/pdbbind'
    
    EF_base   = '/data/fan_zg/MDocking/EcDock_Evaluate/EF/EF-case'
    data_base = '/data/fan_zg/MDocking/VSDS_DTEBV-D/data'
    EF_case_analy(atom_list, save_path, flag_name)
