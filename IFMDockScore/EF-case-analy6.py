...
def EF_case_analy(active_dict, predict_TopN_dict, EF_base, data_base, flag_name = None):
    ...
    bar(active_num_atoms_list, save_file, x_title = 'The number of atoms', bins = 10)
    ...
    os.makedirs(save_path, exist_ok=True)
    save_file = os.path.join(save_path, 'all_atom_number')
    bar(predict_num_atoms_list, save_file, x_title = 'The number of atoms', bins = 10)
...
if __name__ == '__main__':
    ...
    EF_dict = {}
    with open(f'{EF_base}/current_best_mmf_ecdock.pickle', 'rb') as f:
        EF_dict = dill.load(f)
