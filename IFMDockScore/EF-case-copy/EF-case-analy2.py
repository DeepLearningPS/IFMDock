...
def bar(atom_counts, save_file, bins = 10):
    # 示例数据
    #atom_counts = [10, 12, 14, 16, 18, 10, 14, 14, 16, 20]

    # 绘制直方图
    plt.hist(atom_counts, bins=10, color="#0d0b6f", edgecolor='black')
    plt.xlabel('Atom number')
    plt.ylabel('Frequency')
    plt.title('Distribution of atom number')
    #plt.show()
    plt.savefig(save_file+'_bar.png', dpi=300, bbox_inches='tight')
...
def box(atom_counts, save_file):
    ...
    # 保存图像
    plt.savefig(save_file+'_box.png', dpi=300, bbox_inches='tight')
    plt.close()  # 关闭图表，防止重复绘制
...
def PCAs(morgan_fps, atom_counts, save_file):
    ...
    plt.title("Molecular similarity after PCA ")
    plt.colorbar(label="Atom number")
    #plt.show()
    plt.savefig(save_file+'_PCA_scatter.png', dpi=300, bbox_inches='tight')
    plt.close()  # 关闭图表，防止重复绘制
...
def KNN(morgan_fps, save_file, n_clusters = 5):
    ...
    plt.xlabel("PCA Component 1")
    plt.ylabel("PCA Component 2")
    plt.title("Molecular similarity after K-means")
    plt.colorbar(label="Cluster")
    #plt.show()
    plt.savefig(save_file+'_KNN_scatter.png', dpi=300, bbox_inches='tight')
    plt.close()  # 关闭图表，防止重复绘制
...
def EF_case_analy():
    ...
    files = {
        #'Glide':               'glide_norefine_rtm.pickle',
        'CarsiDock':           'carsidock.pickle',

        #'SurfDock':            'surfdock.pickle',
        #'ECDock+GlideScore':            'current_best_mmf_ecdock_norefine_glide.pickle',
        'EC-Dock':              'current_best_mmf_ecdock.pickle',
    }
    EF_dict = {}
    for name, fname in files.items():
        print(f'{EF_base}/{fname}')
        with open(f'{EF_base}/{fname}', 'rb') as f:
            EF_dict[name] = dill.load(f)
    ...
    '''两个字典相减，取绝对值，然后排序，找间隔大的，分析分子量等指标'''
    
    
    for name in EF_dict:
        
        print(len(EF_dict[name]))
    ...
    models = list(EF_dict.keys())
    print('models:', models) #['CarsiDock', 'EC-Dock']

    for name in EF_dict[models[0]].keys():
        new_EF_dict[name] = EF_dict[models[1]][name] - EF_dict[models[0]][name] # ec-dock - carsidock
                    
    sorted_new_EF_dict = dict(sorted(new_EF_dict.items(), key=lambda item: item[1], reverse=True))


    print(sorted_new_EF_dict)
    
    with open(f'{EF_base}/sorted_EF_diff.txt', 'w') as f:
        for name, val in sorted_new_EF_dict.items():
            f.write(f"{name}\t{val}\n")
    ...
    #pos_diff_names = ['P49841-1O6L', 'P08172-6U1N', 'P11362-5EW8']
    #neg_diff_names = ['P10415-6GL8', 'Q07817-6VWC', 'P00338-5W8J']
    ...
    for name in pos_num_atoms_dict:
        save_path = f'{EF_base}/pos/{name}'
        os.makedirs(save_path, exist_ok=True)
        save_file = os.path.join(save_path, name)
        bar(pos_num_atoms_dict[name], save_file, bins = 10)
        box(pos_num_atoms_dict[name], save_file)
        
        PCAs(pos_morgan_fp_dict[name], pos_num_atoms_dict[name], save_file)
        KNN(pos_morgan_fp_dict[name], save_file, n_clusters = 7)
        


    for name in neg_num_atoms_dict:
        save_path = f'{EF_base}/neg/{name}'
        os.makedirs(save_path, exist_ok=True)
        save_file = os.path.join(save_path, name)
        bar(neg_num_atoms_dict[name], save_file, bins = 10)
        box(neg_num_atoms_dict[name], save_file)
        
        PCAs(neg_morgan_fp_dict[name], neg_num_atoms_dict[name], save_file)
        KNN(neg_morgan_fp_dict[name], save_file, n_clusters = 7) 
