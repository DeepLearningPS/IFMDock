...
def run_an_eval_epoch(model, data_loader, pred=False, atom_contribution=False, res_contribution=False, dist_threhold=None, aux_weight=0.001, device='cuda'):
    ...
    with th.no_grad():
        for batch_id, batch_data in enumerate(data_loader):
            ...
            pi, sigma, mu, dist, atom_types, bond_types, batch = model(bgp, bgl) # pi高斯混合系数，mu均值， sigma标准差
            #dist是预测出来的？还是输入配体和蛋白的距离呢，确认一下？应该是后者。神经网络是根据距离学习一组高斯混合密度函数的参数。
...
def calculate_probablity(pi, sigma, mu, y):
    #pi, sigma, mu, dist：pi高斯混合系数，mu均值， sigma标准差, y是预测的dist
    normal = Normal(mu, sigma) #构建一个正态分布,即密度函数
    logprob = normal.log_prob(y.expand_as(normal.loc)) #把距离y代入，得到此时的对数密度值
    logprob += th.log(pi) #加上混合系数 #对数相加等于对数乘法，即log(p1*p2)=log(p1)+log(p2)
    prob = logprob.exp().sum(1) #去对数，得到密度值，然后相加，得到打分。注意不是概率，而是密度，值可以大于1。
    ...
    return prob #得到的是负的密度值，即打分，取绝对值后，值越大，打分越高，则置信度越高
