    ...
    for dt in tqdm(list(sorted(list(os.listdir(dt_dir))))[:]):
        ...
        if os.path.exists(path) and os.path.isdir(path) and os.listdir(path) and 'model' not in dt: #目录存在且不空
            if inargs.gen_pocket:
                ...
                p_file = os.path.join(path, 'step4', f'{dt}_protein.pdb')
            else:
            ...
            if inargs.refine_flag == 'refine':
                ...
                l_file = os.path.join(path, 'step4', f'glide_refine_gen_{dt}.sdf')
                ...
                #l_file = os.path.join(path, f'gen_{dt}_ligand.sdf') 
                l_file = os.path.join(path, 'step4', f'gen_ligand_{dt}.sdf')
            ...
            if inargs.refine_flag == 'refine':
                rtm_score_l_file   = os.path.join(path,  'step4', f'refine_rtm_sort_gen_{dt}_ligand.sdf')
            else:
                rtm_score_l_file   = os.path.join(path,  'step4', f'rtm_sort_gen_{dt}_ligand.sdf')
            
            ref_lig_file = l_file
            
            if inargs.refine_flag == 'refine':
                if inargs.atom_contribution:
                    out_file = os.path.join(path,  'step4', f'refine_{dt}_rtmscore_ac')
                elif inargs.res_contribution:
                    out_file = os.path.join(path,  'step4', f'refine_{dt}_rtmscore_rc')
                else:
                    out_file = os.path.join(path,  'step4', f'refine_{dt}_rtmscore')
            else:
                if inargs.atom_contribution:
                    out_file = os.path.join(path,  'step4', f'{dt}_rtmscore_ac')
                elif inargs.res_contribution:
                    out_file = os.path.join(path,  'step4', f'{dt}_rtmscore_rc')
                else:
                    out_file = os.path.join(path,  'step4', f'{dt}_rtmscore')
            ...
            try:
                ...
                if is_file_exist_and_not_empty(out_file + '.csv'):
                    ...
                    with open(out_file + '.csv') as f:
                        for i in f:
                            refine_index_list.append(i.strip('\n').split('\t')[0])
                            refine_value_list.append(int(float(i.strip('\n').split('\t')[1])))
                            nums += 1
                    
                    if inargs.refine_flag == 'refine':
                        norefine_index_list = []
                        norefine_value_list = []
                        with open(os.path.join(path,  'step4', f'{dt}_rtmscore') + '.csv') as f:
                            for i in f:
                                norefine_index_list.append(i.strip('\n').split('\t')[0])
                                norefine_value_list.append(int(float(i.strip('\n').split('\t')[1])))
                    ...
                    if inargs.cover:
                        ...
                        print('out_file:', out_file + '.csv')
                        print(f'{ligand_num} != {nums}')
                        one_data(p_file, l_file, ref_lig_file, out_file, origin_gen_l_file, rtm_score_l_file, inargs)
                        
                    elif inargs.refine_flag == 'refine' and refine_index_list == norefine_index_list and refine_value_list == norefine_value_list:
                        print('out_file:', out_file + '.csv')
                        print('refine and norefine 是一样的')
                        one_data(p_file, l_file, ref_lig_file, out_file, origin_gen_l_file, rtm_score_l_file, inargs)
