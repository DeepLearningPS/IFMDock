...
def get_index(atom_num, hybridization, is_aromatic, mode):
    ...
    elif mode == 'add_aromatic': #默认用的是这个
...
class FeaturizeProteinAtom(object):
    ...
    def __init__(self):
        ...
        self.max_num_aa = 20 #这个要改，之前是20，现在是43，现在有一个问题，如果使用非标准的氨基酸，则要改氨基酸的数量，但这样模型要重新训练。解决办法去除非标准氨基酸
    ...
    def __call__(self, data: ProteinLigandData):
        ...
        amino_acid = F.one_hot(data.protein_atom_to_aa_type, num_classes=self.max_num_aa) #原子对应的氨基酸残基也one-hot化
...
class FeaturizeLigandAtom(object):
    ...
    def __call__(self, data: ProteinLigandData):
        ...
        '''
        aromatic_list = [v[AROMATIC_FEAT_MAP_IDX] for v in data.ligand_atom_feature] #data.ligand_atom_feature.shape = N * 8, 所以这里获取的
        #是第三列，这个用来判断是否是芳香原子，如果是，则为1，否则为0，data.ligand_atom_feature存放的是ATOM_FAMILIES，是one-hot化

        #因为，我们默认是添加芳香原子的，所以这里要判断，该原子是否是芳香原子
        ...
        '''
        #因为有添加了芳香原子，所以，原子数量由8个变成了13个
        x = [get_index(e, h, a, self.mode) for e, h, a in zip(element_list, hybridization_list, aromatic_list)]
        x = torch.tensor(x)
        data.ligand_atom_feature_full = x #返回的是原子在欧one-hot里面的下标，并没有立刻one-hot化，这一点不同于蛋白质
        return data
