...
#tmp.set_start_method('spawn', force=True)  # 必须在主模块中设置
import math
...
from filelock import FileLock  # 用于文件操作同步
...
# 放在模块顶层（类外部），以便 multiprocessing 可以 pickle 成功
import os
import shutil
import pickle
import traceback
...
class PDBPairDataset(Dataset): #from torch.utils.data import Dataset 不是来自于PyG
    ...
    def __init__(self, raw_path, data_flag='new_test', single_test = False, protein = None, ligand = None, data_name = 'tmpdata', cross_distance_num = None, \
        ...
        self.st_i = st_i 
        self.end_i = end_i
        #v3是基于距离矩阵的，v4是基于3d坐标的，v32是测试集中20个中去最好的训练
    ...
    def _connect_db(self):
        ...
        self.db = lmdb.open(
            self.processed_path,
            map_size=30*(1024*1024*1024),   # 100GB #这个可能开小的，导致数据没有写进去.是的。如果数据量很大，务必把数据库给扩大
    ...
    def _process_single(self):
        ...
        with db.begin(write=True, buffers=True) as txn:
            ...
            #try:
                ...
                #扩大数据范围，在3.5~4.5范围内，找对应蛋白的原子，然后再取这些蛋白原子附近的2ai范围内的原子，形成一个扩充的蛋白集合，记得去重复，排除unimol距离已有的原子，
                #这个在神经实现比较好
                ...
                #无距离矩阵的配体-蛋白连接表, 针对Bindingnet v2
                '''
                pocket_dict = PDBProtein(os.path.join(data_prefix, pocket_fn), ligand_centor, ligand_dict, self.data_flag, self.cross_distance_num, unimol_pcoords = 
                ...
                #从坐标中获取配体到蛋白的连接表，即之前从距离矩阵获取作用信息，现在从3D坐标, 不用扩充
                #pocket_dict = PDBProtein(os.path.join(data_prefix, pocket_fn), ligand_centor, ligand_dict, self.data_flag, self.cross_distance_num, unimol_pcoords = 
    ...
    def _process_mutiple(self):
        ...
        for i, (pocket_fn, protein_fn, (pka, year, resl), ligand_fn, pdbid) in enumerate(tqdm(index[:])): #先使用前100个
            ...
            if pocket_fn is None:
                ...
                i_list.append(i)
                pocket_fn_list.append(pocket_fn) #使用全蛋白
