...
class eSCN(BaseModel):
    ...
    Args:
        use_pbc (bool):         Use periodic boundary conditions #使用周期边界条件 #研究一下，如何使用，可能会改进KNN图的邻居采样
        regress_forces (bool):  Compute forces ##计算力场
        ...
        sphere_channels (int):        Number of spherical channels (one set per resolution) #球坐标的嵌入维度
    ...
    def __init__(
        ...
        use_pbc: bool = True,        #使用周期边界条件
        regress_forces: bool = True, #计算力场
        otf_graph: bool = False,     #是否动态生成计算图，在输入变长的情况下，效果好?这个参数看下是否影响速度
        ...
        if self.distance_function == "gaussian":
            self.distance_expansion = GaussianSmearing(
                0.0,
                cutoff,
                #self.num_gaussians, #8.0/0.02 = 400 #这里的维度需要修改，扩充之后和其他嵌入保持一致
                int(self.num_gaussians / 8),  #50, #400/8 = 50, 8是原子类型的数量
    ...
    def forward(self,         
        ...
        #edge_rot_mat = self._init_edge_rot_mat(None, None, edge_distance_vec) #pos, edge_index, 因为这俩个参数是有用的，错误输出时有用，因此不要传入None
        edge_rot_mat = self._init_edge_rot_mat(pos, edge_index, edge_distance_vec) #pos, edge_index, 因为这俩个参数是有用的，错误输出时有用，因此不要传入None
    ...
    def _init_edge_rot_mat(self, pos, edge_index, edge_distance_vec):
        ...
        if torch.min(edge_vec_0_distance) < 0.0001:
            ...
            print('edge_index is None or []?:', edge_index) #edge_index是None
            logging.error(
        ...
        try:
            assert torch.max(vec_dot) < 0.99 #存在个别情况是这样的，这里注释掉算了
