...
class SO2EquivariantGraphAttention(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.use_atom_edge_embedding:
            ...
            #nn.init.uniform_(self.edge_feat_embedding.weight.data, -0.001, 0.001)
    ...
    def forward(
        ...
        if self.use_atom_edge_embedding:
            source_feats = feats[edge_index[0]]  # Source atom atomic number
            target_feats = feats[edge_index[1]]  # Target atom atomic number
            
            source_embedding = self.source_embedding(source_feats)
            target_embedding = self.target_embedding(target_feats)
            edge_embedding = self.edge_feat_embedding(edge_feats)
            
            x_edge = torch.cat((edge_distance, source_embedding, target_embedding, edge_embedding), dim=1)
        else:
