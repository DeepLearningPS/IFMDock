...
from .MMFF_supply import dic_atom2lin
...
def add_batch_info(dic_MMFF_param):
    ...
    for k in dic_MMFF_param.keys():
        if 'index' in k:
            ...
            dic_tmp[t+'_mask']=[1]*len_t
            #print (k, len(dic_MMFF_param[k]),len(dic_tmp[t+'_batch']))
    return {**dic_MMFF_param, **dic_tmp}
...
def combine_MMFF_params_from_mols(mols):
    mmff_params={}
    mol_pts=[]
    pt=0
    mol_params=[]
    for mid,mol in enumerate(mols):
        dict_MMFF_param=get_MMFF_param(mol)
        mol_atoms=mol.GetNumAtoms()

        for k,v in dict_MMFF_param.items():
            if 'index' in k or 'batch' in k:
                dict_MMFF_param[k]=torch.from_numpy(v).long()
            elif 'mask' in k:
                dict_MMFF_param[k]=torch.from_numpy(v).bool()
            else:
                dict_MMFF_param[k]=torch.from_numpy(v).float()
                
        for key,pad_dim in zip(MMFF_keys,MMFF_pad_dim):

            if len(dict_MMFF_param[key+'_index'])==0:
                dict_MMFF_param[key+'_index']=torch.zeros(pad_dim[0]).long()
                dict_MMFF_param[key+'_param']=torch.zeros(pad_dim[1]).float()
                dict_MMFF_param[key+'_batch']=torch.zeros(1).long()
                dict_MMFF_param[key+'_mask']=torch.zeros(1).bool()
                #print (key,dict_MMFF_param[key+'_index'])
        mol_params.append(dict_MMFF_param)
        mol_pts.append(pt)
        pt+=mol_atoms

    for key in MMFF_keys:
        MMFF_index_list=[]
        MMFF_param_list=[]
        MMFF_batch_list=[]
        MMFF_mask_list=[]
        for i,g in enumerate(mol_params):
            MMFF_index_list.append(g[key+'_index']+mol_pts[i])
            MMFF_param_list.append(g[key+'_param'])
            MMFF_batch_list.append(g[key+'_batch']+i)
            MMFF_mask_list.append(g[key+'_mask'])
        mmff_params[key+'_index']=torch.cat(MMFF_index_list)
        mmff_params[key+'_param']=torch.cat(MMFF_param_list)
        mmff_params[key+'_batch']=torch.cat(MMFF_batch_list)
        mmff_params[key+'_mask']=torch.cat(MMFF_mask_list)
    return mmff_params

def combine_coords_with_masks(coords,masks=None):
    if masks is None:
        masks=torch.ones(*coords.shape[:-1])
    coords_combine=coords.view(-1,3)
    masks_combine=masks.view(-1).bool()
    coords_selected=coords_combine[masks_combine]
    return coords_selected

def combine_fix_masks_with_masks(fix_masks,masks=None):
    if masks is None:
        masks=torch.ones(*fix_masks.shape[:-1])
    fix_masks_combine=fix_masks.view(-1,3)
    masks_combine=masks.view(-1).bool()
    fix_masks_selected=fix_masks_combine[masks_combine]
    return fix_masks_selected

class MMFF_Energy():
    def __init__(self, split_interact=False, warm=False):
        super(MMFF_Energy, self).__init__()
    ...
    def __call__(self, coor, MMFF_param, return_sum=True):
        ...
        dic_e = {}
        dic_e['BS'] = self.get_BS(coor, MMFF_param['BS_index'], MMFF_param['BS_param'], MMFF_param['BS_batch'], MMFF_param['BS_mask'])
        dic_e['AB'] = self.get_AB(coor, MMFF_param['AB_index'], MMFF_param['AB_param'], MMFF_param['AB_batch'], MMFF_param['AB_mask'])
        dic_e['SB'] = self.get_SB(coor, MMFF_param['SB_index'], MMFF_param['SB_param'], MMFF_param['SB_batch'], MMFF_param['SB_mask'])
        dic_e['OOP'] = self.get_OOP(coor, MMFF_param['OOP_index'], MMFF_param['OOP_param'], MMFF_param['OOP_batch'], MMFF_param['OOP_mask'])
        dic_e['TOR'] = self.get_TOR(coor, MMFF_param['TOR_index'], MMFF_param['TOR_param'], MMFF_param['TOR_batch'], MMFF_param['TOR_mask'])
        if self.split_interact:
            dic_e['VDW1'] = self.get_VDW(coor, MMFF_param['VDW1_index'], MMFF_param['VDW1_param'], MMFF_param['VDW1_batch'], MMFF_param['VDW1_mask'])
            dic_e['VDW2'] = self.get_VDW(coor, MMFF_param['VDW2_index'], MMFF_param['VDW2_param'], MMFF_param['VDW2_batch'], MMFF_param['VDW2_mask'])
            dic_e['ELE1'] = self.get_ELE(coor, MMFF_param['ELE1_index'], MMFF_param['ELE1_param'], MMFF_param['ELE1_batch'], MMFF_param['ELE1_mask'])
            dic_e['ELE2'] = self.get_ELE(coor, MMFF_param['ELE2_index'], MMFF_param['ELE2_param'], MMFF_param['ELE2_batch'], MMFF_param['ELE2_mask'])
        else:
            dic_e['VDW'] = self.get_VDW(coor, MMFF_param['VDW_index'], MMFF_param['VDW_param'], MMFF_param['VDW_batch'], MMFF_param['VDW_mask'])
            dic_e['ELE'] = self.get_ELE(coor, MMFF_param['ELE_index'], MMFF_param['ELE_param'], MMFF_param['ELE_batch'], MMFF_param['ELE_mask'])
    ...
    def get_BS(self, coor, BS_index, BS_param, BS_batch, BS_mask):
        '''
        bond stretching
        drij = rij - rij0
        energy = A*drij**2 * (B + C*drij + D*drij**2)
        ...
        :return: energy
        '''
        BS_index=BS_index[BS_mask]
        BS_param=BS_param[BS_mask]
        BS_batch=BS_batch[BS_mask]
    ...
    def get_AB(self, coor, AB_index, AB_param, AB_batch, AB_mask):
        '''
        angle bending
        dtheta = theta - theta0 (degree)
        lin1 = 1 if center atom is not linear else 0
        ...
        :return: energy
        '''
        AB_index=AB_index[AB_mask]
        AB_param=AB_param[AB_mask]
        AB_batch=AB_batch[AB_mask]
    ...
    def get_SB(self, coor, SB_index, SB_param, SB_batch, SB_mask):
        '''
        stretch-bend
        drij = rij - r0ij
        drkj = rkj - r0kj
        ...
        :return: energy
        '''
        SB_index=SB_index[SB_mask]
        SB_param=SB_param[SB_mask]
        SB_batch=SB_batch[SB_mask]
    ...
    def get_OOP(self, coor, OOP_index, OOP_param, OOP_batch, OOP_mask):
        '''
        out-of-plane bending,
        chi: (degree)
        energy = A*chi**2
        ...
        :return: energy
        '''
        OOP_index=OOP_index[OOP_mask]
        OOP_param=OOP_param[OOP_mask]
        OOP_batch=OOP_batch[OOP_mask]
    ...
    def get_TOR(self, coor, TOR_index, TOR_param, TOR_batch, TOR_mask):
        '''
        torsion
        cos1: cos 1 dihedral
        cos2: cos 2 dihedral
        ...
        :return: energy
        '''
        TOR_index=TOR_index[TOR_mask]
        TOR_param=TOR_param[TOR_mask]
        TOR_batch=TOR_batch[TOR_mask]
    ...
    def get_VDW(self, coor, VDW_index, VDW_param, VDW_batch, VDW_mask):
        '''
        van der Waals
        rij: ...
        energy = A / (rij + B)**7 * (C / (rij**7 + D) + E)
        ...
        :return: energy
        '''
        VDW_index=VDW_index[VDW_mask]
        VDW_param=VDW_param[VDW_mask]
        VDW_batch=VDW_batch[VDW_mask]
    ...
    def get_ELE(self, coor, ELE_index, ELE_param, ELE_batch, ELE_mask):
        '''
        electrostatic
        qi: partial charge of node i
        qi: partial charge of node j
        ...
        :return: energy
        '''
        ELE_index=ELE_index[ELE_mask]
        ELE_param=ELE_param[ELE_mask]
        ELE_batch=ELE_batch[ELE_mask]
