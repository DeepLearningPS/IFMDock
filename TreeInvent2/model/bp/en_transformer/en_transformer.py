...
class Residual(nn.Module):
    ...
    def forward(self, feats, coors, flexible_labels, **kwargs):
        feats_out, coors_delta = self.fn(feats, coors, flexible_labels, **kwargs)
        return feats + feats_out, coors + coors_delta * (flexible_labels.unsqueeze(-1))
...
class FeedForward(nn.Module):
    ...
    def forward(self, feats, coors,flexible_labels):
...
class Block(nn.Module):
    ...
    def forward(self, inp, coor_changes = None):
        feats, coors, flexible_labels, mask, edges, adj_mat  = inp
        feats, coors = self.attn(feats, coors, flexible_labels=flexible_labels, edges = edges, mask = mask, adj_mat = adj_mat)
        feats, coors = self.ff(feats, coors,flexible_labels)
        return (feats, coors,flexible_labels, mask, edges, adj_mat)
...
class EnTransformer(nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        if exists(self.sigma_emb):
            sigma_embed=self.sigma_emb(sigmas.view(-1,1)).unsqueeze(1)
            feats=feats+sigma_embed
        ...
        coor_changes = [coors]
        inp = (feats, coors, flexible_labels, mask, edges, adj_mat)
