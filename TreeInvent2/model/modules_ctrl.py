...
class Node_adder_ctrl(torch.nn.Module):
    def __init__(self):
        ...
        if FGP.with_ctrlnet_for_nadd:
            ...
            self.c_ctrl_net_atom_zero_mlp=zero_module(torch.nn.Linear(FGP.graph_hidden_dim,FGP.graph_hidden_dim*2))
            self.c_ctrl_net_graph_zero_mlp=zero_module(torch.nn.Linear(FGP.graph_hidden_dim,FGP.graph_hidden_dim*2))

        self.c_molgnn=GGNN_Net()
    ...
    def forward(self,complex_nodes,complex_edges,complex_coords,complex_masks):
        ...
        unctrl_atom_embedding=torch.cat((c_atom_embedding[:,FGP.max_patoms:],l_atom_embedding),dim=-1)
    
        unctrl_graph_embedding=torch.cat((c_graph_embedding,l_graph_embedding),dim=-1)
...
class Ring_gener_ctrl(torch.nn.Module):
    ...
    def forward(self,complex_nodes,complex_edges,complex_coords,complex_masks,ring_nodes,ring_edges,f_t,ring_masks):
        ...
        unctrl_atom_emb=torch.cat((c_atom_emb[:,FGP.max_patoms:],l_atom_emb),dim=-1)*complex_masks[:,FGP.max_patoms:].unsqueeze(-1).long()
        unctrl_graph_emb=torch.cat((c_graph_emb,l_graph_emb),dim=-1)
        if FGP.with_ctrlnet_for_rgen:
...
class Node_connect_ctrl(torch.nn.Module):
    def __init__(self):
        ...
        self.node_connect_net_2 = MLP(
            in_features=64*FGP.max_latoms+FGP.graph_hidden_dim*4,
            hidden_layer_sizes=[FGP.graph_hidden_dim] * FGP.graph_depth,
    ...
    def forward(self,complex_nodes,complex_edges,complex_coords,complex_masks,ring_nodes,ring_edges,focused_ids,ring_masks):
        ...
        unctrl_cond_graph_emb=torch.cat((c_graph_emb,l_graph_emb),dim=-1)
        #print (ring_nodes.shape,ring_edges.shape)
        r_graph_emb,r_atom_emb=self.r_net(ring_nodes.float(),ring_edges.float())
        
        focused_atom_embedding=batched_index_select(r_atom_emb,focused_ids.long()).squeeze(1)
        
        unctrl_ligand_atom_emb=torch.cat((c_atom_emb[:,FGP.max_patoms:],l_atom_emb),dim=-1)
