...
class Set2Vec(torch.nn.Module):
    ...
    def forward(self, hidden_output_nodes : torch.Tensor, input_nodes : torch.Tensor,
        ...
        lstm_input   = torch.zeros(batch_size, self.memory_size, device=self.device)
        cat          = torch.cat((hidden_output_nodes, input_nodes), dim=2)
        memory       = self.embedding_matrix(cat)
        hidden_state = torch.zeros(batch_size, self.memory_size, device=self.device)
        cell_state   = torch.zeros(batch_size, self.memory_size, device=self.device)
