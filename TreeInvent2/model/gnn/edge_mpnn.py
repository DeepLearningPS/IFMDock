...
class EdgeMPNN(torch.nn.Module):
    ...
    """
    def __init__(self,edge_features,edge_embedding_size,message_passes,max_n_nodes,device) -> None:
        super().__init__()
    ...
    def forward(self, nodes : torch.Tensor, edges : torch.Tensor) -> torch.Tensor:
        ...
        r = torch.arange(1, n_edges + 1, device=self.device)
        ...
        node_sets = torch.zeros(batch_size,
                                n_nodes,
                                max_node_degree,
                                self.edge_embedding_size).to(nodes)
