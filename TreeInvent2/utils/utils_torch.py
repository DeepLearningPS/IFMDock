...
def kabsch_algorithm(P, Q, pos_mask=None):
    # Center the point clouds
    if pos_mask is None:
        pos_mask = torch.ones(P.shape[:2], device=P.device)
    else:
        if pos_mask.shape[0] != P.shape[0]:
            raise ValueError("pos_mask should have same number of rows as number of input vectors.")
        if pos_mask.shape[1] != P.shape[1]:
            raise ValueError("pos_mask should have same number of cols as number of input vector dimensions.")
        if pos_mask.ndim != 2:
            raise ValueError("pos_mask should be 2 dimensional.")
    denom = torch.sum(pos_mask, dim=1, keepdim=True)
    ...
    Q_mean = torch.sum(Q*pos_mask[:,:,None], dim=1, keepdim=True)/denom[:,:,None]
    ...
    H = torch.bmm(Q_centered.transpose(1, 2), pos_mask[:,:,None]*P_centered)
    H=H+torch.randn_like(H)/1E8
    # Compute the rotation matrix using eigenvalue decomposition
    U, _, Vt = torch.svd(H)
    R = torch.bmm(Vt, U.transpose(1, 2))
    R=R+torch.randn_like(R)/1E8
    # Ensure proper rotation matrix (preserve orientation)
    det = torch.det(R)
    mask = torch.ones(det.shape, device=P.device)
    mask[det < 0] = -1
