    ...
    def __init__(self):
        ...
        ----
            constants (namedtuple) : Contains job parameters as well as global
                                     constants.
        """
        self.score_components = FGP.rlsetting.score_components # list
        self.score_weights = FGP.rlsetting.score_weights
        self.score_type       = FGP.rlsetting.score_type        # list
        self.target_smiles    = FGP.rlsetting.target_smiles
        self.qsar_models_path = FGP.rlsetting.qsar_models_path  # dict
        self.device           = FGP.modelsetting.device
        self.max_gen_atoms      = FGP.rlsetting.max_gen_atoms
        self.score_thresholds = FGP.rlsetting.score_thresholds
        self.tanimoto_k    = FGP.rlsetting.tanimoto_k
        ...
        #self.dock_output_path=FGP.docksetting.output_path

        #self.target_smarts = FGP.rlsetting.target_smarts
        ...
        if "shape" in self.score_components:
            self.rocs_model=Rocs_scorer(shape_w=FGP.rocssetting.shape_w,
                                        color_w=FGP.rocssetting.color_w,
                                        cff_path=FGP.rocssetting.cff_path,
                                        sdf_path=FGP.rocssetting.reflig_sdf_path,
                                        sim_measure=FGP.rocssetting.sim_measure
                                        )
        ...
        for score_component in self.score_components:
            if "target_size" in score_component:
                ...
                assert target_size <= self.max_atoms, \
                       "Target size > largest possible size (`FGP.syssetting.max_atoms`)."
