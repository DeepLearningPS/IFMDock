...
class TreeInvent_Sampler:
    ...
    def update_single_atom_r_graphs(self,nadd_mol_ids,nadd_type_ids,single_node_ids):
        ...
        self.r_masks[nadd_mol_ids[single_node_ids],0]=1
        return 
    ...
    def update_r_graphs_with_add_actions(self,batch,bond_to,atom_type,charge,bond_type,bond_from,likelihoods):
        ...
        self.r_adjs_mat[batch_non_empty,
                        ...
                        bond_type_non_empty+1]=1
        for bid in batch:
            self.r_masks[bid,:self.r_atomnums[bid]]=1
    ...
    def sample_mols(self,MG,nmols_per_complex=10,savepath='./sample_mols',debug=True):
        ...
        for bid,Datas in sample_bar: 
            ...
            while torch.sum(self.l_stop_mask[1:])>0:
                ...
                if debug:
                    print ("NADD stage"+'-'*80)
                    vars=MG.sample_leaf_nadd_states(stepid=nadd_step_id,max_patoms=FGP.max_patoms,max_latoms=FGP.max_latoms)
                    c_atoms_onehot_ref,c_fcs_onehot_ref=self.__to_device(vars[1]),self.__to_device(vars[3])
                    c_adjs_mat_ref=self.__to_device(vars[5])
                    c_feats_ref=torch.cat((c_atoms_onehot_ref,c_fcs_onehot_ref),dim=-1)
                    
                    print ('nadd counter',nadd_step_id,'*'*80)
                    print ('c_feats_diff',torch.sum(torch.abs(c_feats_ref-c_feats[1])).cpu().numpy())
                    print('c_adjs_mat_diff',torch.sum(torch.abs(c_adjs_mat_ref-c_adjs_mat[1])).cpu().numpy())
                    c_coords_ref=self.__to_device(vars[6])
                    print ('c_coords_diff',torch.sum(torch.abs(c_coords_ref[:FGP.max_patoms]-c_coords[1][:FGP.max_patoms])).cpu().numpy())
                    
                    nadd_apds_ref=self.__to_device(vars[-1]).unsqueeze(0)
                    nadd_action_ref=self.model.sample_actions(nadd_apds_ref)
                    nadd_idc_ref,term_idc_ref=self.model.split_nadd_action(nadd_action_ref)
                    print ('ref_action',nadd_idc_ref,term_idc_ref)
                
                
                l_add_ids,l_term_ids,nadd_likelihoods=self.model.nadd_step(c_feats,c_adjs_mat,c_coords,c_masks)
                if debug:
                    print ('pred_action',l_add_ids,l_term_ids)
                ...
                while torch.sum(self.r_stop_mask[1:])>0:
                    ...
                    if debug:
                        vars=MG.sample_leaf_rgen_states(stepid=rgen_step_id,max_patoms=FGP.max_patoms,max_latoms=FGP.max_latoms,max_ringsize=FGP.max_lgroup_size)
                        c_atoms_onehot_ref,c_fcs_onehot_ref=self.__to_device(vars[1]),self.__to_device(vars[3])
                        c_adjs_mat_ref=self.__to_device(vars[5])
                        print ('counter',rgen_step_id,'*'*80)
                        print ('c_feats_diff',torch.sum(torch.abs(c_feats_ref-c_feats[1])).cpu().numpy())
                        print('c_adjs_mat_diff',torch.sum(torch.abs(c_adjs_mat_ref[FGP.max_patoms:,FGP.max_patoms:]-c_adjs_mat[1][FGP.max_patoms:,FGP.max_patoms:])).cpu().numpy())
                        A,B,C=torch.where(torch.abs(c_adjs_mat_ref[FGP.max_patoms:,FGP.max_patoms:]-c_adjs_mat[1][FGP.max_patoms:,FGP.max_patoms:])>0)
                        print ('Diff in ligand adjs:',A.cpu().numpy(),B.cpu().numpy(),C.cpu().numpy())
                        print ('Diff value in ligand adjs:',c_adjs_mat_ref[FGP.max_patoms:,FGP.max_patoms:][A,B,C].cpu().numpy(),c_adjs_mat[1][FGP.max_patoms:,FGP.max_patoms:][A,B,C].cpu().numpy())
                        r_atoms_onehot_ref,r_fcs_onehot_ref=self.__to_device(vars[9]),self.__to_device(vars[11])
                        r_feats_ref=torch.cat((r_atoms_onehot_ref,r_fcs_onehot_ref),dim=-1)
                        r_adjs_mat_ref=self.__to_device(vars[13])
                        r_masks_ref=self.__to_device(torch.Tensor(vars[-3]))
                        print ('r_feats_diff',torch.sum(torch.abs(r_feats_ref-r_feats[1])).cpu().numpy())
                        print('r_adjs_mat_diff',torch.sum(torch.abs(r_adjs_mat_ref-r_adjs_mat[1])).cpu().numpy())
                        print ('r_mask_diff',torch.sum(torch.abs(r_masks_ref-r_masks[1])).cpu().numpy())
                        print ('r_masks_ref',r_masks_ref)
                        print ('r_masks_pred',r_masks[1])
                        D,E,F=torch.where(torch.abs(r_adjs_mat_ref-r_adjs_mat[1])>0)
                        print ('Diff in ring adjs:',D.cpu().numpy(),E.cpu().numpy(),F.cpu().numpy())
                        print ('Diff value in ring adjs:',r_adjs_mat_ref[D,E,F].cpu().numpy(),r_adjs_mat[1][D,E,F].cpu().numpy())
                        c_coords_ref=self.__to_device(vars[6])
                        print ('c_coords_diff',torch.sum(torch.abs(c_coords_ref-c_coords[1])).cpu().numpy())
                        
                        rgen_apds_ref=self.__to_device(vars[-1]).unsqueeze(0)
                        rgen_action_ref=self.model.sample_actions(rgen_apds_ref)
                        radd_idc_ref,rconn_idc_ref,rterm_idc_ref=self.model.split_rgen_action(rgen_action_ref)
                        print ('ref_action',radd_idc_ref,rconn_idc_ref,rterm_idc_ref)
                        rmols,rsmis,rvids=self.graph_to_mols(r_feats_ref.unsqueeze(0),r_adjs_mat_ref.unsqueeze(0),atomnums=torch.sum(r_masks_ref.unsqueeze(0),dim=-1).long())
                        for mol in rmols:
                            Draw.MolToFile(mol,f"./{rgen_step_id}.png")
                    # sample rgen actions
                    ...
                    if debug:
                        print('pred_action',r_add_ids,r_conn_ids,r_term_ids)
                ...
                if conf_step_id>0:
                    if debug:
                        ...
                        print ('counter',nconn_step_id,'*'*80)
                        vars=MG.sample_leaf_nconn_states(stepid=nconn_step_id,max_patoms=FGP.max_patoms,
                        ...
                        print ('c_feats_diff',torch.sum(torch.abs(c_feats_ref-c_feats[1])))
                        print('c_adjs_mat_diff',torch.sum(torch.abs(c_adjs_mat_ref-c_adjs_mat[1])))
                 
                        nconn_apds_ref=self.__to_device(vars[-1]).unsqueeze(0)
                        nconn_action_ref=self.model.sample_actions(nconn_apds_ref)
                        nconn_idc_ref=self.model.split_nconn_action(nconn_action_ref)
                        print ('ref_action',nconn_idc_ref)
                ...
                while torch.sum(self.nint_stop_mask[1:])>0:
                    ...
                    if debug:
                        vars=MG.sample_leaf_nint_states(stepid=nint_step_id,max_patoms=FGP.max_patoms,
                                                        max_latoms=FGP.max_latoms,
                        ...
                        cg_atoms_onehot_ref,cg_fcs_onehot_ref=self.__to_device(vars[1]),self.__to_device(vars[3])
                        cg_feats_ref=torch.cat((cg_atoms_onehot_ref,cg_fcs_onehot_ref),dim=-1)
                        cg_adjs_mat_ref=self.__to_device(vars[5])
                        print ('counter',nint_step_id,'*'*80)
                        print ('cg_feats_diff',torch.sum(torch.abs(cg_feats_ref-cg_feats[1])))
                        print('cg_adjs_mat_diff',torch.sum(torch.abs(cg_adjs_mat_ref[:FGP.max_patoms,FGP.max_patoms:]-cg_adjs_mat[1,:FGP.max_patoms,FGP.max_patoms:])))
                        nint_apds_ref=self.__to_device(vars[-1]).unsqueeze(0)
                        nint_action_ref=self.model.sample_actions(nint_apds_ref)
                        nint_idc_ref,term_idc_ref=self.model.split_nint_action(nint_action_ref)
                        print ('ref_action',nint_idc_ref,term_idc_ref)
                    ...
                    if debug: 
                        print ('pred_action',nint_ids,nint_term_ids)
                ...
                if True:
                    ...
                    if debug:
                        vars=MG.sample_l_coord_gen_states(stepid=conf_step_id,max_patoms=FGP.max_patoms,
                                                        max_latoms=FGP.max_latoms,
                        ...
                        c_feats_ref=torch.cat((c_atoms_onehot_ref,c_fcs_onehot_ref),dim=-1)
                        c_adjs_mat_ref=self.__to_device(vars[5])
                        coords_ref=self.__to_device(vars[6])
                        print ('c_feats_diff in coords',torch.sum(torch.abs(c_feats_ref-c_feats[1])))
                        print ('c_adjs_diff in coords',torch.sum(torch.abs(c_adjs_mat_ref-c_adjs_mat[1])))
